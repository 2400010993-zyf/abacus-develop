...
{
    const unsigned int magic_number = 10000;
    if (seed_in == 0 || seed_in == -1)
    {
        srand(static_cast<unsigned int>(time(nullptr)) + GlobalV::MY_RANK * magic_number); // GlobalV global variables are reserved
    }
    else
    {
        srand(static_cast<unsigned int>(std::abs(seed_in)) + (GlobalV::MY_BNDGROUP * GlobalV::NPROC_IN_BNDGROUP + GlobalV::RANK_IN_BPGROUP) * magic_number);
    }
